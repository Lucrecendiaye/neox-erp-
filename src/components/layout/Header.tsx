...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b border-surface-200 h-16">
      <div className="flex items-center justify-between h-full px-4 lg:px-6">
        <div className="flex items-center gap-2 sm:gap-4">
          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="flex items-center justify-center w-10 h-10 rounded-lg hover:bg-surface-100 text-surface-400 transition-colors lg:hidden"
          >
          ...
          <div className="hidden sm:block">
            <h1 className="text-lg font-bold text-surface-900">Tableau de bord</h1>
            <p className="text-xs text-surface-400">{new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-2 rounded-lg bg-surface-100 hover:bg-surface-200 text-surface-400 transition-colors min-w-[40px] sm:min-w-[200px] text-sm"
          >
            <Search className="w-4 h-4 shrink-0" />
            <span className="hidden sm:inline">Rechercher...</span>
            <kbd className="hidden sm:inline ml-auto text-[10px] px-1.5 py-0.5 rounded bg-surface-200 text-surface-400 font-mono">Ctrl+K</kbd>
          </button>
          ...
          <button className="relative w-10 h-10 rounded-lg hover:bg-surface-100 flex items-center justify-center text-surface-400 transition-colors">
            <Bell className="w-4.5 h-4.5" />
            <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-primary-500 text-white text-[8px] font-bold rounded-full flex items-center justify-center shadow-sm">3</span>
          </button>

          <div className="relative" ref={userRef}>
            <button
              onClick={() => setUserDropdown(!userDropdown)}
              className="flex items-center gap-2 pl-3 border-l border-surface-200 hover:opacity-80 transition-opacity"
            >
              <div className="w-8 h-8 bg-primary-500 text-white rounded-full flex items-center justify-center text-sm font-semibold shadow-sm">
