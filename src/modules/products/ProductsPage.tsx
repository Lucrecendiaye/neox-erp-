...
export default function ProductsPage() {
  ...
  const rawProducts = (isCloud ? supabaseProducts : dexieProducts) || ([] as Product[])
  ...
  const rawStocks = (isCloud ? supabaseStocks : dexieStocks) || ([] as ProductStock[])

  const dexieSales = useLiveQuery(() => db.sales.where('businessId').equals(businessId).toArray(), [businessId])
  const { data: supabaseSales } = useSupabaseQuery<any>('sales', undefined, [])
    ...
    switch (pinAction.type) {
      ...
      case 'delete_category': {
        ...
        try {
          ...
          if (isCloud) { await sb.remove('categories', catId) } else { await db.categories.delete(catId) }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editing) {
        ...
        if (isCloud) {
          await sb.insert('products', product as any)
        } else {
        ...
        if (initialStock > 0) {
          ...
          if (isCloud) {
            await sb.insert('stock_movements', movement as any)
            await sb.insert('product_stocks', {
  ...
  async function confirmDeleteProduct(id: string) {
    ...
    if (isCloud) { await sb.remove('products', id) } else { await db.products.delete(id) }
  ...
  async function handleStockAdjust() {
    ...
    if (isCloud) {
      await sb.insert('stock_movements', movement as any)
      const existingStocks = await sb.filter<any>('product_stocks', 'productId', stockAdjust.productId)
      const matched = existingStocks?.find((s: any) => s.locationId === shopId)
      if (matched) {
        await sb.update('product_stocks', matched.id, { quantity: stockAdjust.quantity, updatedAt: now } as any)
      } else {
        await sb.insert('product_stocks', {
  ...
  async function handleSaveCategory() {
    const now = new Date().toISOString()
    if (catEdit) {
      if (isCloud) { await sb.update('categories', catEdit.id, { ...catForm, updatedAt: now } as any) } else { await db.categories.update(catEdit.id, { ...catForm, updatedAt: now } as any) }
      toast('Catégorie modifiée', 'success')
    } else {
      const cat = { id: generateId(), businessId, ...catForm, createdAt: now }
      if (isCloud) { await sb.insert('categories', cat as any) } else { await db.categories.add(cat as any) }
    ...
    <div className="w-full h-full flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 w-full">
        {paginatedItems?.map((p) => {
          ...
          return (
            <div key={p.id} className="bg-white rounded-2xl border border-surface-200 shadow-sm hover:shadow-md hover:border-primary-200 transition-all group flex flex-col">
              {/* Image */}
              <div className="relative h-32 bg-surface-50 rounded-t-2xl flex items-center justify-center overflow-hidden">
                {p.photos?.[0] ? (
                ...
                {isOut && (
                  <span className="absolute top-2 left-2 px-2 py-0.5 rounded-md bg-red-500 text-[10px] font-semibold text-white">Rupture</span>
                )}
                {isLow && !isOut && (
                  <span className="absolute top-2 left-2 px-2 py-0.5 rounded-md bg-amber-500 text-[10px] font-semibold text-white">Stock faible</span>
                )}
                {p.status === 'inactive' && (
                  <span className="absolute top-2 right-2 px-2 py-0.5 rounded-md bg-surface-500 text-[10px] font-semibold text-white">Masqué</span>
                )}
              </div>
              ...
              <div className="p-4 flex-1 flex flex-col gap-1.5">
                <div className="min-w-0">
                  ...
                  {catName && <p className="text-[11px] text-surface-400 truncate">{catName}</p>}
                </div>

                <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs mt-1">
                  <span className="text-surface-500">Prix vente</span>
                  <span className="text-surface-900 font-medium text-right">{formatCurrency(p.sellingPrice)}</span>
                  {(p.wholesalePrice || 0) > 0 && (
                    <>
                      <span className="text-surface-500">Prix gros</span>
                      <span className="text-surface-900 font-medium text-right">{formatCurrency(p.wholesalePrice!)}</span>
                    </>
                  )}
                  <span className="text-surface-500">Stock</span>
                  <span className={cn('font-medium text-right', isOut ? 'text-red-600' : isLow ? 'text-amber-600' : 'text-surface-900')}>
                    {qty}
                    <span className="text-surface-400 font-normal"> pièces</span>
                  </span>
                  <span className="text-surface-500">Seuil stock</span>
                  <span className="text-surface-900 font-medium text-right">{alert}</span>
                  <span className="text-surface-500">Coût stock</span>
                  ...
                  <span className="text-surface-500">Vendus</span>
                  <span className="text-surface-900 font-medium text-right">{soldQty}</span>
                </div>
              </div>

              {/* Actions */}
              <div className="flex items-center justify-between px-4 py-2.5 border-t border-surface-100 gap-1">
                <div className="flex gap-1">
                  {can('edit') && (
                    <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-primary-600 transition-colors" title="Modifier">
                      <Edit2 className="w-3.5 h-3.5" />
                    </button>
                  )}
                  {can('view') && (
                    <button onClick={() => navigate(`/products/${p.id}`)} className="p-1.5 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-blue-600 transition-colors" title="Fiche détaillée">
                      <Eye className="w-3.5 h-3.5" />
                    </button>
                  )}
                  {can('edit') && (
                    <button onClick={() => openStockAdjust(p)} className="p-1.5 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-amber-600 transition-colors" title="Ajuster le stock">
                      <PackageOpen className="w-3.5 h-3.5" />
                    </button>
                  )}
                  <button onClick={() => navigate(`/products/${p.id}`)} className="p-1.5 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-purple-600 transition-colors" title="Historique">
                    <History className="w-3.5 h-3.5" />
                  </button>
                  {can('delete') && (
                    <button onClick={() => handleDelete(p.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-surface-400 hover:text-red-600 transition-colors" title="Supprimer">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  )}
                </div>
                <span className="text-[10px] text-surface-400">{p.barcode || p.reference || '—'}</span>
              </div>
            </div>
          )
        })}
      ...
      {/* Product Modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Modifier le produit' : 'Nouveau produit'} size="xl">
