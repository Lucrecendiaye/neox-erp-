...
async function main() {
  ...
  // First, delete any existing IndexedDB by navigating to the page
  await page.goto('https://neox-erp-alpha.vercel.app/', { waitUntil: 'domcontentloaded' });
  await page.evaluate(() => new Promise(resolve => {
    const r = indexedDB.deleteDatabase('neox_erp');
    r.onsuccess = () => resolve();
    r.onerror = () => resolve();
  }));
  await new Promise(r => setTimeout(r, 1000));

  // Now seed using raw IndexedDB
  ...
  const result = await page.evaluate(async (productData) => {
    ...
    // Set localStorage BEFORE creating the database so the app can find them
    localStorage.setItem('neox-user-id', userId);
    localStorage.setItem('neox-session-start', now);

    async function hashPassword(password) {
      const encoder = new TextEncoder();
      const data = encoder.encode(password + 'neox-salt-v1');
      ...
      r.onsuccess = () => {
        ...
        function idbAdd(storeName, data) {
          return new Promise((res, rej) => {
            ...
            if (Array.isArray(data)) data.forEach(d => store.add(d));
            else store.add(data);
            tx.oncomplete = () => res();
            tx.onerror = (e) => rej(e.target.error?.message || 'error');
          });
        }

        async function run() {
          await idbAdd('settings', { id: 'default', name: 'Application', currency: 'XOF', currencySymbol: 'FCFA', currencies: [{ code: 'XOF', symbol: 'FCFA', rate: 1, isDefault: true }, { code: 'EUR', symbol: '€', rate: 0.0015 }, { code: 'USD', symbol: '$', rate: 0.0016 }], locale: 'fr-FR', language: 'fr', timezone: 'Africa/Ouagadougou', taxRate: 18, invoicePrefix: 'FAC-', invoiceNextNumber: 1 });
          await idbAdd('businesses', { id: bizId, name: "Admin Boutique's Shop", currency: 'XOF', currencySymbol: 'FCFA', phone: '+22670123456', email: 'admin@boutique.com', isActive: true, createdAt: now });
          await idbAdd('users', { id: userId, businessId: bizId, name: 'Admin Boutique', email: 'admin@boutique.com', phone: '+22670123456', passwordHash: hash, role: 'admin', permissions: ['*'], isActive: true, createdAt: now });
          await idbAdd('locations', [
            { id: shopLocId, businessId: bizId, name: 'Boutique Principale', type: 'shop', address: '', phone: '+22670123456', isActive: true, createdAt: now, updatedAt: now },
            { id: warehouseLocId, businessId: bizId, name: 'Dépôt Principal', type: 'warehouse', address: '', phone: '', isActive: true, createdAt: now, updatedAt: now },
          ]);
          await idbAdd('accounts', [
          ...
          for (const name of catNames) {
            const id = genId();
            catMap[name] = id;
            await idbAdd('categories', { id, businessId: bizId, name, parentId: '', createdAt: now });
          }
          ...
          for (const p of productData) {
            ...
            await idbAdd('products', { id: prodId, businessId: bizId, name, description: '', barcode, reference: barcode, categoryId: catMap[cat] || '', brand, unit, purchasePrice, sellingPrice, wholesalePrice, priceDozen: 0, pricePack: 0, packSize: unit === 'pack' ? (packSize || 6) : undefined, taxRate, stockAlert, location: '', photos: [], margin, status: 'active', createdAt: now, updatedAt: now });

            const q1 = Math.floor(Math.random() * 10) + 5;
            await idbAdd('productStocks', { id: genId(), businessId: bizId, productId: prodId, locationId: shopLocId, quantity: q1, stockAlert, stockMin: 0, stockMax: 0, updatedAt: now });
            await idbAdd('stockMovements', { id: genId(), businessId: bizId, locationId: shopLocId, productId: prodId, type: 'in', quantity: q1, unitPrice: purchasePrice, reference: 'INIT', note: 'Stock initial boutique', createdAt: now, userId });

            const q2 = Math.floor(Math.random() * 20) + 10;
            await idbAdd('productStocks', { id: genId(), businessId: bizId, productId: prodId, locationId: warehouseLocId, quantity: q2, stockAlert, stockMin: 0, stockMax: 0, updatedAt: now });
            await idbAdd('stockMovements', { id: genId(), businessId: bizId, locationId: warehouseLocId, productId: prodId, type: 'in', quantity: q2, unitPrice: purchasePrice, reference: 'INIT', note: 'Stock initial dépôt', createdAt: now, userId });
          }

          return { success: true, bizId, userId, productCount: productData.length };
        }

        run().then(resolve).catch(err => resolve({ error: err.message }));
      };
      r.onerror = () => resolve({ error: 'Failed to open' });
    });
  }, PRODUCTS);
  ...
  if (result.success) {
    console.log('Reloading page for session pickup...');
  ...
  await context.close();
  console.log('\n✅ Compte: admin@boutique.com / admin123');
