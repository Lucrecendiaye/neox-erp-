...
async function main() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (hasErr) { await page.locator('button:has-text("Réessayer")').click(); await new Promise(r => setTimeout(r, 3000)); }
    else break;
  }
  ...
  console.log('Seeding with business:', biz.id);
  ...
  const result = await page.evaluate(async ({ PRODUCTS, CAT_NAMES, businessId, userId, locs }) => {
    ...
    for (const name of CAT_NAMES) { const id = genId(); catMap[name] = id; await db.categories.add({ id, businessId, name, parentId: '', createdAt: now }); }
    ...
    return { products: await db.products.count(), categories: await db.categories.count(), stocks: await db.productStocks.count(), movements: await db.stockMovements.count() };
  ...
  console.log('🌍 Deployed: https://neox-erp-alpha.vercel.app');
  console.log('(pour le site déployé: node scripts/seed-deployed.mjs)');
}
