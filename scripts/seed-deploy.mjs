...
async function main() {
  ...
  console.log('1. Inscription sur le site déployé...');
  await page.goto(`${BASE}/register`, { waitUntil: 'networkidle' });
  await sleep(3000);

  const body = await page.evaluate(() => document.body.innerText.substring(0, 300));
  if (body.includes('Erreur')) {
    console.log('Erreur détectée, tentative de clic sur Réessayer...');
  ...
  const businessId = dbInfo.biz.id;
  const userId = dbInfo.user.id;
  const shopLoc = dbInfo.locs.find(l => l.type === 'shop');
  const warehouseLoc = dbInfo.locs.find(l => l.type === 'warehouse');
  console.log(`Business: ${businessId}, Shop: ${shopLoc.id}, Warehouse: ${warehouseLoc.id}`);

  console.log('2. Création des catégories et produits...');
      ...
      r.onsuccess = () => {
        ...
        catTx.oncomplete = () => {
          ...
          PRODUCTS.forEach(p => {
            ...
            prodStore.add({ id: prodId, businessId, name: p.name, description: '', barcode: p.barcode, reference: p.barcode, categoryId: catMap[p.cat] || '', brand: p.brand, unit: p.unit, purchasePrice: p.purchasePrice, sellingPrice: p.sellingPrice, wholesalePrice: p.wholesalePrice || 0, priceDozen: 0, pricePack: 0, packSize, taxRate: p.taxRate, stockAlert: p.stockAlert || 5, location: '', photos: [], margin, status: 'active', createdAt: now, updatedAt: now });
          ...
          tx.onerror = (e) => resolve('tx error: ' + e.target.error?.message);
        };
        catTx.onerror = (e) => resolve('cat error: ' + e.target.error?.message);
      };
      r.onerror = () => resolve('db open error');
    });
  }, { CATEGORIES, PRODUCTS, businessId, shopLocId: shopLoc.id, warehouseLocId: warehouseLoc.id, userId });
  ...
  const counts = await page.evaluate(() => new Promise((resolve) => {
    ...
    r.onsuccess = () => {
      ...
      tx.objectStore('stockMovements').count().onsuccess = e => { m = e.target.result; resolve({ produits: p, categories: c, stocks: s, mouvements: m }); };
    };
  }));
  console.log('État final:', counts);
  ...
  console.log('========================================');
  console.log('✅ 30 produits créés (pièce/douzaine/paquet)');
  console.log('✅ 8 catégories');
  console.log('✅ Stock dans Boutique + Dépôt Principal');
  console.log('========================================\n');
}
